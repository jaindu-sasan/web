...
export function Footer() {
  return (
    <>
      <footer className="border-t border-white/10 bg-[#020812] px-4 pb-8 pt-16 text-white sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-12 border-b border-white/10 pb-14 md:grid-cols-2 lg:grid-cols-[1.5fr_1fr_1fr_1.2fr]">
            ...
            <div><h3 className="footer-title">Get in touch</h3><div className="space-y-4 text-sm text-slate-400"><a href="mailto:info@trustcode.com" className="flex items-center gap-3 hover:text-cyan-400"><Mail size={16}/>info@trustcode.com</a><a href="tel:+1234567890" className="flex items-center gap-3 hover:text-cyan-400"><Phone size={16}/>+1 (234) 567-890</a><p className="flex items-start gap-3"><MapPin size={16} className="mt-1 shrink-0"/>123 Tech Street, Innovation City, IC 12345</p></div></div>
          </div>
      ...
      <a href="https://wa.me/1234567890" target="_blank" rel="noopener noreferrer" aria-label="Chat with Trust Code on WhatsApp" className="fixed bottom-5 right-5 z-40 grid h-14 w-14 place-items-center rounded-full bg-[#25D366] text-white shadow-xl transition-transform hover:scale-110 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-cyan-400"><MessageCircle size={25}/></a>
    </>
  )
}
